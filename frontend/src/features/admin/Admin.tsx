...
interface RecordItem { details?: Record<string, string>; term_ids?: number[]; id: number; title: string; type: string; status: string; author_id: number; locale: string; slug: string; summary: string; body: string; organization: string; source_url: string; closing_date: string | null; expires_at: string | null; }
...
export default function Admin() {
  ...
  return <div className="admin-workspace"><div className="admin-heading"><div><span className="eyebrow">EDITORIAL WORKSPACE</span><h1>Content &amp; administration</h1><p>{user.name} <span className="role-badge">{user.roles.join(', ')}</span></p></div><button className="secondary" onClick={() => logout.mutate()} disabled={logout.isPending}>Sign out</button></div>{logout.error && <p className="admin-error" role="alert">{logout.error.message}</p>}<nav className="admin-tabs" aria-label="Workspace sections">{[['content', 'Content', 'cms.access'], ['advertisements', 'Advertisement imports', user.permissions.includes('media.manage') ? 'media.manage' : 'media.upload'], ['taxonomy', 'Taxonomy', 'taxonomy.manage'], ['operations', 'Operations', 'operations.view'], ['tools', 'Manage tools', 'tools.manage'], ['users', 'Users & roles', 'users.manage'], ['roles', 'Permission matrix', 'roles.view'], ['audit', 'Audit history', 'audit.view']].filter(([, , permission]) => user.permissions.includes(permission)).map(([key, label]) => <button key={key} className={tab === key ? 'selected' : ''} onClick={() => setTab(key)}>{label}</button>)}</nav>{tab === 'content' && <ContentWorkspace user={user} />}{tab === 'advertisements' && <Advertisements user={user} />}{tab === 'taxonomy' && <Taxonomy />}{tab === 'operations' && <Operations />}{tab === 'tools' && <ToolManagement />}{tab === 'users' && user.permissions.includes('users.manage') && <Users />}{tab === 'roles' && user.permissions.includes('roles.view') && <Roles />}{tab === 'audit' && user.permissions.includes('audit.view') && <Audit />}</div>;
}
...
function ContentWorkspace({ user }: { user: StaffUser }) {
  ...
  function actions(item: RecordItem): string[] {
    ...
    if (item.status === 'draft' && permitted('update') && (item.author_id === user.id || user.permissions.includes('users.manage'))) return ['submit'];
...
function ContentEditor({ item, user, close, saved }: { item: RecordItem | 'new'; user: StaffUser; close: () => void; saved: () => void }) {
  ...
  const canEdit = !record || (record.status === 'draft' && user.permissions.includes(`${record.type}.update`) && (record.author_id === user.id || user.permissions.includes('users.manage')));
