...
test('exam update desk separates categories, switches language and refreshes notices', async ({ page }) => {
  ...
  await page.goto('/');
  const board = page.locator('.classic-home');
  ...
  await board.getByRole('group', { name: 'Notice language', exact: true }).getByRole('radio', { name: 'Hindi', exact: true }).check();
  await expect(board.getByText(/No published .* in Hindi yet/)).toHaveCount(7);
  await expect(board.getByRole('link', { name: /^View all results/ })).toHaveAttribute('href', '/results?locale=hi');
