...
const env = { ...process.env, APP_ENV: 'testing', APP_KEY: `base64:${randomBytes(32).toString('base64')}`, DB_CONNECTION: 'sqlite', DB_DATABASE: database, SESSION_DRIVER: 'file', CACHE_STORE: 'file', MAIL_MAILER: 'log', APP_URL: 'http://127.0.0.1:8005' };
...
const frontend = spawn(process.execPath, ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5175', '--strictPort'], { cwd: resolve(root, 'frontend'), env: { ...process.env, PORTAL_API_TARGET: 'http://127.0.0.1:8005' }, stdio: 'ignore' });
...
try {
  ...
  const child = spawn(process.execPath, ['node_modules/@playwright/test/cli.js', 'test'], { cwd: resolve(root, 'frontend'), stdio: 'inherit' });
